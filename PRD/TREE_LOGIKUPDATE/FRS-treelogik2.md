...
- setup3.md
- README.md
- dem Manual des genealogytree package als Referenz fuer allgemeine Layout-Prinzipien genealogischer Baeume

Prioritaetsregel bei Inkonsistenzen:

- Wenn dieses Dokument den produktfachlichen Zielen aus den obigen Quellen widerspricht, haben die produktfachlichen Quellen Vorrang.
- Wenn dieses Dokument allgemeinen Layout-Prinzipien fuer genealogische Baumdarstellung widerspricht, hat das genealogytree-Manual Vorrang.
- Fuer Preview3 und seine Nachfolge-Implementierung ist das genealogytree-Manual damit die primaere Referenz fuer Ebenenlogik, Familienpositionierung, Abstandssemantik und strukturelle Reservierungen.
...
- Die Layout-Engine arbeitet deterministisch und rasterbasiert, nicht als frei verschiebendes Kraft- oder Heuristik-Layout.
- Die Tree-Logik verwendet mindestens zwei logisch getrennte Raster:
   1. ein Personenraster fuer Personenknoten
   2. ein Familien- oder Junction-Raster zwischen Generationen fuer Eltern-Kind-Verknuepfungen, Partnerachsen und Familienmittelpunkte
- Eine Person belegt fachlich genau eine Personenzelle.
- Ein Familienknoten oder Junction-Punkt belegt fachlich genau einen reservierten Slot im Familienraster.
- Personenraster und Familienraster sind gekoppelt, aber nicht identisch.

### 2.1.1 Raster- und Zellprinzip

- Die Generationslogik wird ueber feste Ebenen modelliert.
- Die sichtbare Personenbox ist nicht identisch mit der Rasterzelle.
- Node-Groesse, Label-Breite und Textlaenge duerfen die fachliche Zellbelegung nicht redefinieren.
- Kollisionen werden primaer durch diskrete Slot-Zuweisung und Breitenreservierung verhindert, nicht durch spaetes weiches Wegschieben.
- Ein Layout gilt fachlich nur dann als stabil, wenn jede sichtbare Person einer eindeutigen Zelle zugeordnet ist und keine zwei Personen dieselbe Zelle beanspruchen.

### 2.1.2 Familienblock-Prinzip

- Die Engine behandelt nicht nur Einzelpersonen, sondern Familienbloecke als eigene Platzierungseinheiten.
- Ein Familienblock umfasst mindestens:
   - die zugehoerigen Eltern
   - ihren gemeinsamen Familien- oder Junction-Punkt
   - die von diesem Punkt abhaengige Kindgruppe
- Die Breite eines Familienblocks wird vor der finalen Platzierung abgeschaetzt und reserviert.
- Familienbloecke werden gegeneinander platziert; Einzelpersonen sind innerhalb des Blocks nachgeordnet.
- House-Cluster und Familienbloecke sind getrennte Ebenen: House-Cluster strukturieren Root-Kontexte, Familienbloecke strukturieren biologische Teilbloecke.

### 2.1.3 Achse und Pivot

- Jeder Familienblock besitzt eine explizite fachliche Mitte oder Pivot-Achse.
- Diese Pivot-Achse wird nicht implizit aus zufaelligen Nachbarverschiebungen abgeleitet.
- Die Pivot-Regel muss ausdruecklich definieren, ob sich eine Familie an der Elternmitte, an der Kindmitte oder an einer kombinierten Mittelachse orientiert.
- Lokale Optimierungen duerfen die definierte Familienachse nicht still brechen.

### 2.1.4 Reservierte Leerslots

- Leere oder unsichtbare Reservierungsslots sind ein legitimes Layout-Mittel.
- Solche Reservierungen duerfen verwendet werden, um Symmetrie, Partnerkontinuitaet, kindlose Familien, Mehrfachpartnerschaften oder breite Kindgruppen lesbar zu halten.
- Reservierte Slots sind fachliche Platzhalter und keine semantischen Personenobjekte.

### 2.1.5 Typisierte Abstaende

- Abstaende werden fachlich typisiert und nicht auf eine einzige globale X-Luecke reduziert.
- Mindestens diese Abstandsklassen sind getrennt definierbar:
   - Geschwisterabstand innerhalb einer Kindgruppe
   - Partnerabstand innerhalb eines Partnerblocks
   - Abstand zwischen Familienbloecken derselben Ebene
   - Abstand zwischen groesseren Kontextbloecken oder Komponenten
- Distanzwerte duerfen je nach Leserichtung und Beziehungstyp unterschiedlich sein, muessen aber deterministisch bleiben.
...
- yOffset darf die Zuordnung zu Personen- oder Familienraster nicht aufloesen, sondern nur verschieben.
- Kuratierte Vertikalverschiebung muss weiterhin an klaren Ebenen andocken; sie ersetzt keine freie Positionierung.
...
- Die House-Cluster-Ebene ersetzt nicht die Familienblock-Ebene; beide muessen getrennt modellierbar bleiben.
...
- Partnerdarstellung darf das Personenraster erweitern oder reservierte Slots belegen, aber nicht die biologische Hauptachse still umdefinieren.
...
- Das Overlay nutzt die vorhandenen Personen- und Familienraster, statt eigene freie Geometrie zu erzwingen.
...
- Die Platzreservierung fuer House-Cluster muss mit der Platzreservierung der darunterliegenden Familienbloecke vereinbar sein.
...
- Die Breitenvorabschaetzung muss mindestens die Blockbreiten der untergeordneten Familienkontexte beruecksichtigen.
...
- Fuer den Modus R2 ist der Zielzustand ein harter Slot-Allocator mit eindeutiger Zellbelegung statt ein spaet korrigiertes Flusslayout.
- Die derzeitigen biologischen Achsen- und Kollisions-Paesse sind Uebergangslogik; fachlich bevorzugt ist eine primäre Block- und Slot-Platzierung.
...
- Die aktuelle Session hat gezeigt, dass ein diskreter Rasteransatz mit eindeutiger Zellbelegung robuste Kollisionsfreiheit herstellen kann; die naechste Ausbaustufe ist die Aufwertung des Familienrasters zur echten Erstklassen-Struktur.
